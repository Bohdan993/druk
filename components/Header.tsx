...
const Header: FC<HeaderProps> = ({data}) => {
    ...
    return (
        <header id="header" className="header bg-natural-green h-[60px] shadow-[4px_4px_10px_rgba(38,38,38,0.2)]">
                    ...
                    <nav className="menu ps-[192px]">
                        <ul className="flex justify-between items-center">
                            {data?.attributes?.menu?.map((el, i, arr) => {
                                ...
                                return (
                                    <li className={`pe-[20px] lg:pe-[50px] last:pe-0 font-[600] lg:font-bold leading-[19px] hidden md:block tracking-[0.2em] ${len > 2 && ((i + 1) === (len - 2) || (i + 1) === (len - 1)) ? "md:hidden lg:block" : ""}`} key={el?.id}>
                        ...
                        <ul className="flex md:justify-center justify-end items-center">
                            ...
                            <li className="lg:hidden">
                                <div className="cursor-pointer" onClick={handleClick}>
                                    <Image src={iconBurger} alt="Icon Burger" />
                                </div>
                            </li>
                        </ul>
                    </div>
