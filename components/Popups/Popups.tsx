...
type PopupsProps = {
  burgerData: {
    menu: IHeader["attributes"]["menu"];
    phone: IHeader["attributes"]["phone"];
    social: IHeader["attributes"]["social"];
  };
};
...
const Popups: FC<PopupsProps> = ({ burgerData }) => {
  ...
  const activePopup = Object.entries(popupsState).filter(
    (el) => el[1]
  )?.[0]?.[0];
  ...
  const ActivePopupWithProps = cloneElement(ActivePopup, {
    id: activePopup,
    onClose: handleClose,
    burgerData,
  });

  const isSomePopupVisible = Object.entries(popupsState)
    .filter((el) => !el[0].includes("Responsive"))
    .some((el) => el[1] === true);
