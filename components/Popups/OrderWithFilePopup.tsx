...
const schema: any = yup
  .object({
    ...
    url: yup.string().when("file-radio", {
      is: (val: string) => val === "Посилання",
      then: (schema) =>
        yup.string().required("Це поле обов'язкове для заповнення"),
    }),
  })
  .required();
  ...
  return (
    ...
    >
          ...
          <div>
            <div className="flex items-center mb-[10px] md:mb-[15px]">
              ...
              <label htmlFor={`${props?.id}-file`} className="block w-full">
                <Dropzone
                  isDisabled={watchFileRadio !== "Файл"}
                  setFile={setFile}
                  filesProp={file}
                />
              </label>
            ...
            <div className="flex items-center">
              ...
              <div className="relative w-full">
                <label
                  htmlFor={`${props?.id}-file2`}
                  className="flex items-center w-full cursor-pointer justify-between md:justify-start"
                >
                  ...
                  <input
                    className={`input font-[600] leading-[1.2] py-[8.5px] px-[15px] text-black border-black border-[1px] rounded-[15px] outline-none text-[14px] w-full max-w-[190px] md:max-w-[173px] ${
                      watchFileRadio !== "Посилання" ? "bg-gray" : ""
                    }`}
                    disabled={watchFileRadio !== "Посилання"}
                    type="text"
                ...
                <p className="text-danger absolute right-0 bottom-[-15px]">
                  {watchFileRadio === "Посилання" && errors?.url?.message}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="pb-[12px] md:pb-[25px] relative">
        <div>
          <label
            className="font-bold text-[1.125rem] leading-[1.2] tracking-[0.2em] mb-[10px] md:mb-[15px] block"
            htmlFor=""
          >
            Додайте коментар:
          </label>
